# Bulk Card Creator with Json — Blueprint
...
| Mục | Chi tiết |
|-----|----------|
| Tên addon | Bulk Card Creator with Json (`meta.json`) |
...
| Menu | **AnkiVN** (ngang hàng Tools) → "Bulk Card Creator with Json" |
...
- `QAction("Bulk Card Creator with Json")` → `show_dialog()` → `BulkCardCreatorDialog(mw).exec()`
...
sequenceDiagram
    ...
    User->>Menu: Bulk Card Creator with Json
...
- **Test thủ công:** Cài addon → restart Anki → mở profile có collection → AnkiVN → Bulk Card Creator with Json.
