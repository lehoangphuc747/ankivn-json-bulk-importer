...
- `config.py` — điểm duy nhất ghi `user_config.json` (media_fields, presets, lang, window_maximized, window_geometry, convert_markdown, selected_stats). Writes atomic qua file `.tmp` + `os.replace`. Preset/history **phải** qua API ở đây, không tự ghi file trong UI.
